...
import { getMRev, getMTx, getMWE } from '@/lib/calculations'
...
export default function LocationDetailClient({ location, machines, locationMachines, snapshots, settings }: Props) {
  ...
  const nebenkosten = location.nebenkosten ?? 0
  ...
  const daysToExpiry = vertragDate ? Math.ceil((vertragDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : null
    ...
    <div style={{ padding: '2rem', maxWidth: 960, margin: '0 auto' }}>
        ...
        <div>
          ...
          {location.adresse && (
            <p style={{ color: 'var(--text-secondary)', margin: '4px 0 0', fontSize: 14 }}>
              {location.adresse}{location.plz || location.ort ? ,  .trim() : ''}
            </p>
          )}
        </div>
      ...
      {showExpiryAlert && (
        <div style={{
          ...
          <div>
            <strong style={{ color: '#fef2f2', fontSize: 14 }}>Vertragsablauf in {daysToExpiry} Tagen</strong>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 16, marginBottom: '1.5rem' }}>
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 120 }}>
          ...
          <svg width={96} height={96} viewBox='0 0 96 96'>
            ...
            <circle cx={48} cy={48} r={38} fill='none' stroke={kiColor} strokeWidth={8}
              strokeLinecap='round'
      ...
      <div style={{ background: 'var(--card-bg)', borderRadius: 12, padding: 24, marginBottom: '1.5rem' }}>
        ...
        <WaterfallRow label='Umsatz' value={totalRev} base={totalRev} color='#3b82f6' isStart />
        <WaterfallRow label={Warenentnahme (%)} value={-totalWE} base={totalRev} color='#f97316' />
        <WaterfallRow label='= DB1' value={db1} base={totalRev} color='#8b5cf6' isSubtotal />
        <WaterfallRow label={Miete ()} value={-miete} base={totalRev} color='#ef4444' />
        {nebenkosten > 0 && <WaterfallRow label={Nebenkosten ()} value={-nebenkosten} base={totalRev} color='#ef4444' />}
        {fixCosts > 0 && <WaterfallRow label={Maschinenkosten ()} value={-fixCosts} base={totalRev} color='#ef4444' />}
        <WaterfallRow label='= DB2 (Standort-Gewinn)' value={db2} base={totalRev} color={db2 >= 0 ? '#22c55e' : '#ef4444'} isSubtotal isEnd />
      </div>

      <div style={{ background: 'var(--card-bg)', borderRadius: 12, padding: 24, marginBottom: '1.5rem' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>Maschinen ({locationMachines.length})</h3>
        {locationMachines.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            ...
            <tbody>{locationMachines.map(m => {
              const rev = getMRev(m)
              const we = rev * getMWE(m, settings.we_rate)
              const d1 = rev - we
              return (
                <tr key={m.id} onClick={() => router.push(/machines/)}
      ...
      <div style={{ background: 'var(--card-bg)', borderRadius: 12, padding: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 14 }}>
          ...
          <DetailRow label='Nebenkosten' value={location.nebenkosten ? fmt(location.nebenkosten) : null} />
          ...
          <div style={{ background: 'var(--card-bg)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              ...
              <button onClick={() => setEditing(false)}
                style={{ flex: 1, background: 'var(--card-bg)', color: 'var(--text-secondary)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 0', cursor: 'pointer' }}
              >Abbrechen</button>
            </div>
          </div>
        </div>
...
function WaterfallRow({ label, value, base, color, isStart, isSubtotal, isEnd }: {
  label: string; value: number; base: number; color: string
  isStart?: boolean; isSubtotal?: boolean; isEnd?: boolean
}) {
  const pct = base > 0 ? Math.abs(value) / base : 0
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0',
      ...
      <div style={{ width: 200, fontSize: 13, color: 'var(--text-secondary)', flexShrink: 0 }}>{label}</div>
      <div style={{ flex: 1, height: 8, background: 'var(--border)', borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ width: ${barW}%, height: '100%', background: color, borderRadius: 4 }} />
      </div>
      <div style={{ width: 90, textAlign: 'right', fontWeight: isSubtotal || isEnd ? 700 : 400, fontSize: 14, color, flexShrink: 0 }}>
        {value >= 0 ? '' : '-'}{new Intl.NumberFormat('de-AT', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(Math.abs(value))}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ gridColumn: colSpan === 2 ? 'span 2' : undefined }}>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg)', color: 'var(--text-primary)',
          border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 14 }}
      />
    </div>
  )
}
